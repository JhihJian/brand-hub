...
| `JWT_ACCESS_EXPIRES_IN` | 否 | 12h | Access Token 有效期 |
...
| POST | `/auth/login` | 用户登录（短信验证码）|
| POST | `/auth/login-password` | 用户登录（密码）|
...
| 429 | RATE_LIMITED | 密码错误次数超限（15分钟）|
...
│       └── password.js    # 密码加密工具
├── scripts/
│   ├── generate-keys.js   # 生成 JWT 密钥对
│   └── set-default-passwords.js  # 设置默认密码
├── tests/
│   └── e2e.test.js        # E2E 测试
├── keys/                   # JWT 密钥（不提交）
├── data/                   # SQLite 数据（不提交）
...
```bash
node scripts/set-default-passwords.js
```

默认密码为 `wszz526418`，请在首次登录后提醒用户修改密码。
